...
Languages with a relative number system that have agglutinative morphology for \textsc{min} indeed exist; the Rembarrnga paradigm in \tabref{tab:01:3} provides an example.
...
We have argued that third person has a feature specification, as opposed to singular number, and explained why nevertheless both can function as defaults. We now show how the asymmetry in feature specification plays out in agreement.
  ...
  \citet{Nevins2011} discusses so-called omnivorous number systems, in which a verb shows plural agreement when either subject or object is plural (see 19)).

\ea \label{bkm:Ref295394739}\label{bkm:Ref295395935}  Eastern Abruzzese (\citealt{D’Alessandro2010})\\
...
Like Nevins, we assume that data like (19) involve \isi{multiple agreement}. We further assume that this leads to a situation in which one morpho-phonological agreement slot must realize two distinct feature bundles: 
...
Agreement with nominative objects in \ili{Icelandic} is possible when the subject carries \isi{quirky case}. However, such agreement is usually impossible with first or second person objects:\footnote{\citet{D’Alessandro2007} shows that \isi{impersonal} \textit{si} constructions in Italian behave in a fashion parallel to the \ili{Icelandic} examples discussed below: \textit{si} triggers default third person singular agreement, and when the object is nominative the verb agrees in number with it. Crucially, in the latter case the object cannot be first or second person. Any adequate analysis proposed for \ili{Icelandic} can therefore be extended to Italian \isi{impersonal} constructions, as indeed argued by D’Alessandro.}
...
Dutch clefts show almost the same pattern of core observations as \ili{Icelandic} quirky subject constructions (see also \citealt{Dikken2014}). They have the following properties. 

\begin{enumerate}[label=(\roman*)]
\item  Number agreement with a clefted nominative is obligatory (see (32)). 
\item  If there is unambiguous \isi{person agreement}, first and second person nominatives cannot be clefted (see (33)). 
\item Some speakers allow suspension of \isi{person agreement} with clefted nominatives. In that case, there is no person restriction (hence the \%-sign on the variants with third singular \textit{is} in (33a,b)). 
...
These data allow an analysis similar to that proposed for \ili{Icelandic}. Dutch requires agreement with the subject and (usually) agreement with nominatives. If the clefted constituent is a nominative DP, this yields the following representation: 
...
\z Apparently, then, \ili{Icelandic} clefts also permit deletion of one of the $\varphi $-feature bundles in the verb before spell-out, but as opposed to the relevant variety of Dutch, it is the agreement with the subject that is suppressed in \ili{Icelandic}, rather than the agreement with the nominative predicate. This gives rise to the question why the same deletion is not allowed in quirky subject constructions. One possibility is that this is related to the fact that the agreement induced by such a subject is \isi{default agreement}. Arguably, \isi{default agreement} cannot be deleted because it is not recoverable, as opposed to regular agreement, which reflects features of the controller.}
...
There is an interesting twist in the plural. Here, all speakers require \isi{number agreement}, but there are no effects of the person restriction:
...
\ea \settowidth\jamwidth{(example: Nocte, see below)}
\ea  2 > 1 > 3  \jambox{(example: Ojibwe, see above)}
\ex  1 > 2 > 3  \jambox{(example: Nocte, see below)}
...
% 
% Ariel, Mira. 2000. The development of \isi{person agreement} markers: From pronouns to higher accessibility markers. In Barlow, Michael \& Kemmer, Suzanne (eds.), \textit{Usage-based models of language}, 197–260. Stanford, CA: CSLI Publications.
% 
