...
Applying this test to \is{pronoun!null possessive}null possessives, F\&N show that \is{pronoun!null possessive}null possessives are restricted to a sloppy reading, but only if the relational noun is in object position, as in (20). But both readings are available if the relational noun is in subject position (a position from which movement cannot take place), as in (21). Once again, in EP there is no subject-object asymmetry. The strict reading is not available either in object or in subject position.
...
In BP, \is{pronoun!null possessive}null possessives cannot take split antecedents, as shown in (27), which corroborates the movement analysis: if the \is{pronoun!null possessive}null possessive is the copy of the antecedent, two independent DPs cannot be its antecedent. Also in EP, \is{pronoun!null possessive}null possessives do not allow for split antecedents. But as shown above, the structural requirements for a movement analysis are not met.
...
It is worth mentioning that EP differs from \ili{English} with respect to this diagnostic. Even if a \isi{reflexive} pronoun occurs in an exempt position, it may not take split antecedents. Only (personal) pronouns can do so. The EP examples are given in (28a) and (28b), respectively.
...
% 
% Kupisch, Tanja \& Rinke, Esther. 2011. The diachronic development of article-possessor complementarity in the history of \ili{Italian} and \ili{Portuguese}. In Siemund, Peter (ed.), \textit{Linguistic universals and language variation}, 92–127. Berlin: De Gruyter Mouton.
% 
...
% 
% Parodi, Claudia. 1994. On Case and agreement in \ili{Spanish} and \ili{English} DPs. In Mazzola, Michael L. (ed.), \textit{Issues and theory in \ili{Romance} linguistics: Selected papers from the Linguistic Symposium on \ili{Romance} Languages XXIII, April 1-4}, 403–416. Washington, D. C.: Georgetown University.
% 
...
% 
% Rizzi, Luigi. 1986. Null objects in \ili{Italian} and the theory of \textit{pro}. \textit{Linguistic Inquiry} 17. 501–557.
% 
