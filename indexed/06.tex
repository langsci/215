...
In languages like Gungbe, there is a progressive particle \textit{tò} which means literally ‘be at’. The lexical verb, when it immediately follows the progressive particle, similarly to what happens in \isi{Conversanese}, may undergo a process of reduplication \citep{Aboh2004,Aboh2009}, as in (21), where \textit{ɖa} is the verb and \textit{ɖiɖa} is its reduplicated form.
...
With transitive verbs, \citet{Laka2006} points out that there is a contrast between canonical transitive sentences, in which the subject receives \isi{ergative case} (25), and their progressive equivalents, in which the subject and the nominalized clause \textit{ogia jaten} (26) receive \isi{absolutive case} (zero marked).
...
These data concerning overt case marking in Basque confirm that progressive structures imply an unaccusativization of the event: when the progressive auxiliary is expressed, the subject is marked with \isi{absolutive case}, as in all \isi{intransitive} (\isi{unaccusative}) structures. Furthermore, the presence of a PP as a complement of the auxiliary supports the crosslinguistic generalization that progressives are \isi{unaccusative} \isi{locative} constructions. The next section is devoted to the analysis of the progressive constructions in \isi{Conversanese} as \isi{locative} constructions. 

\section{A syntactic analysis of the progressive inflected constructions}% 4. 
\subsection{Introduction}% 4.1. 
The main progressive construction in \isi{Conversanese}, which we introduced in §1 and §2 and is repeated here in (27), is formed from an inflected \isi{stative} verb \textit{stɛ} (=‘to stay’), a \isi{locative} \isi{preposition} \textit{a} and an inflected lexical verb. It patterns with the \isi{unaccusative} \isi{locative} construction (28) formed from a \isi{stative} auxiliary and a \isi{locative} phrase. 
...
In \isi{Conversanese}, there is a parallel progressive construction that we introduced in §1 and §2 and is repeated here in (39). It is formed from an inflected \isi{stative} verb \textit{stɛ} (=‘to stay’), the \isi{locative} \isi{preposition} \textit{a} and an \isi{uninflected} lexical verb (\isi{infinitive}). It differs from the aspectual inflected construction mainly in its syntactic structure and aspectual entailment. 
...
Like the aspectual inflected progressive (30), it allows only long-distance \isi{clitic placement}, (40). But since the embedded verb is an \isi{infinitive}, it allows enclitics, (41), which are not possible with the finite verbs in the inflected aspectual counterpart. 
