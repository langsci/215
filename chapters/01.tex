...
for tree={no edge,l sep=0mm,l=0mm,s sep=7mm}
[{\textbf{\textit{i}} (\textit{a\textsubscript{i}})},tikz={\node [inner sep=0pt,ellipse,draw,fit=()] {};}
         [{S\textit{\textsubscript{i}}},tikz={\node [ellipse,inner sep=0pt,draw, fit=() (!s) (!u)] {};\node [inner sep=0pt,ellipse,draw, fit=() (!1) (o) (!u)] {};} [
          ...
          [{\textbf{\textit{u}} (\textit{a\textsubscript{u}})} [] [{(\textit{o})},name=o]]
]
\end{forest}}
...
\end{forest}
\z
\todo[inline]{Is ``Generic ONE'' the root node or the heading in this tree?}
...
\z
\z
\todo[inline]{I swapped the order of the equivalence in c. to make it more readable}
...
\begin{enumerate}[label=(\roman*)]
\item  Number agreement with a clefted nominative is obligatory (see (35)). 
\item  If there is unambiguous person agreement, first and second person nominatives cannot be clefted (see (36)). 
\item Some speakers allow suspension of person agreement with clefted nominatives. In that case, there is no person restriction (hence the \%-sign on the variants with third singular \textit{is} in (36a,b)). 
\item  Where the verb forms triggered by the pronoun in subject position (\textit{het} ‘it’) and by the clefted nominative DP are identical, the person restriction is lifted for all speakers. This is the case with some modal verbs and in the past tense (see (37)).
\end{enumerate}
...
\lspbottomrule
\end{tabularx}
\todo[inline]{please confirm table}
\end{table}
...
\ex  1,2 > 3    \jambox{(example: Kaqchikel, see below)}
\z
\z
\todo[inline]{Specify the exs you are refering to?}
