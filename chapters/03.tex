...
\ea%6
    (de \citealt{Benito2010}: 8, 14)\todo{this is the source for both maps?}
    \label{ex:gallego:6}\\
...
Notice that, in both SE and SE-less cases, agreement is only in number, not person (cf. \citealt{Etxepare2005}), but there seems to be robust evidence that we are dealing with syntactic LDA.\footnote{%
    ...
    \z
    \z\todo[inline]{Lopez is the source for a--b?}} 
...
\judgewidth{?*}
    \label{ex:gallego:12}
    \ea[?]{
    \gll Me       \textbf{faltaron}      [ corregir \textbf{esos   exámenes} ].\\
         to.me   lacked-\textsc{3pl}   mark      those exams\\
    \glt ‘I couldn’t mark those exams.’}
    \ex[?*]{
    \gll Me      \textbf{faltaron}   [ haber  corregido \textbf{esos    exámenes} ].\\
...
\ea%25
    \label{ex:gallego:25}
    \ea Leísta Spanish\\
     Dialect A:   [\textit{\textsubscript{v}}\textsubscript{P} \textit{v} [\textsubscript{VP} V  [\textsubscript{PP} \ConnectTail{\textit{a}}   [ DP\textsubscript{OBLIQUE} \ConnectHead{]} ] ] ] 
...
These data have not been described in reference grammars of Spanish (cf. \citealt{Bosque1999}; RAE-ASALE 2009), plausibly because they can be can be regarded as production errors. The data have, however, also been reported by the Syntactic Atlas of Spanish (ASinEs) (see \figref{ex:gallego:30}).
...
\ea%31
    ...
    \ea[*]{
    \gll \textbf{Fueron}  \textbf{habladas}          con   \textbf{las  autoridades}.  \\
         be{}-\textsc{3pl}   talked{}-\textsc{f.3pl} with  the  authorities\\
    \glt ‘The authorities were spoken to.’  }
    \ex[*]{
    \gll  \textbf{Fueron  informados}              de  \textbf{cuatro homicidios}. \\
...
\ea%36
    ...
    \ea[*]{
    \gll Se  \textbf{trabajaron}  en  \textbf{las reuniones}.   \\
         \textsc{se}   work-\textsc{3pl}     in  the meetings\\
    \glt ‘People worked in the meetings.’  }
    \ex[*]{
    \gll Se   \textbf{criticaron}     al              Presidente  por \textbf{varias razones}. \\
