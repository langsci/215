...
This difference in interpretation depends on the adjunction site of the adverb. When it modifies the matrix verb, it (right-)adjoins to the matrix vP or TP \REF{ex:alexiadou:10a}. When it modifies the embedded verb, it adjoins to the embedded vP or TP \REF{ex:alexiadou:10b}:
...
Evidence from \textit{negative concord} potentially suggests that in BC the subject does \emph{not} belong to the higher clause and surface to the right of the embedded verb as a result of rightward scrambling. Negative quantifiers in Greek, a negative concord language, must be either in the clause containing sentential negation \REF{ex:alexiadou:11a} or in the c-command domain of a higher sentential negation \REF{ex:alexiadou:11b}. They cannot be licensed by a negation in a lower clause \REF{ex:alexiadou:11c} (see \citealt{Giannakidou1997}):
...
Negative concord points to the existence of a higher copy in BC. If such a copy wasn’t present, \REF{ex:alexiadou:12d} should be fully acceptable. Further evidence in support of this comes from the observation that in Greek, \textit{nominal secondary predicates and predicative modifiers} like ‘alone’ agree in gender and number with the c-commanding DP they modify:
...
Note that, as well as examples like \REF{ex:alexiadou:6} where the embedded subject is nominative, native speakers were also asked to evaluate examples like \REF{ex:alexiadou:15} below involving BC between an embedded dative\slash genitive or accusative experiencer and a matrix null (nominative) subject. 
...
\citet{Tsakali2017} show that the Greek facts are very different: specifically, there is no blocking of VSO orders and BC in OC constructions involving a filled SpecCP; cf. \REF{ex:alexiadou:21}:
