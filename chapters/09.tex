...
=======
    \gll \relax [\textsubscript{PP} De  qui]  est-ce que [\textsubscript{DP}  la  secrétaire  \sout{[\textsubscript{PP} de qui]}      t’a  téléphoné? \\
>>>>>>> 2c69ffccf31cee16d51ec971c7b4ad9df445f303
...
\ea%2
    \label{ex:mensch:2}
    \ea[*]{
    \gll \relax[\textsubscript{PP} Sur qui]  as-tu  lu  [\textsubscript{DP}  le  livre \sout{[\textsubscript{PP}  sur qui]}] ?\\
       {}  on  whom  have-you  read {} the  book {}  on whom\\
    \glt ‘On whom have you read the book \_\_’?   }
    \ex[*]{
    \gll \relax[\textsubscript{PP}  A qui]    avez-vous vu [\textsubscript{DP}  une  amie \sout{[\textsubscript{PP}  a qui]}] ?\\
        {} to whom  have-you seen {}  a  friend {}   to whom\\
    \glt ‘Of whom have you seen a friend \_\_? (similar to \citealt{Grosu1974}: 312, Footnote 3)  }
    \ex[]{
    \gll \relax[\textsubscript{PP}  De  qui]     avez-vous vu  [\textsubscript{DP} une  photo \sout{[\textsubscript{PP}  de  qui]}] ?\\
        {} of  whom  have-you seen {} a  photo  {}  of  whom\\
    \glt ‘Of whom have you seen a photo \_\_?’  }
    \ex[]{
    \gll \relax[\textsubscript{PP}  De quel livre]  connais-tu [\textsubscript{DP}  la  fin   \sout{[\textsubscript{PP} de quel livre]}] ?\\
...
\ea%3
    \label{ex:mensch:3}
    \ea[]{
    \gll le  linguiste [\textsubscript{PP} duquel/dont]\footnotemark{}  tu  as  lu  le  livre [\textsubscript{DP} \sout{[\textsubscript{PP}        duquel/dont]}]\\
             the  linguist {} of.which  you  have  read  the  book      {} {}   of.which\\
    \glt     ‘the linguist of (= by) whom you have read the book \_\_’}
    \ex[*]{
    \gll  le  linguiste [\textsubscript{PP}  sur lequel / sur qui]  tu  as  lu  le  livre  [\textsubscript{DP} \sout{[\textsubscript{PP}  sur lequel / sur qui]}]\\
...
As we shall see, this first rough approximation needs some refinement, and, in addition, problems arise when the DP contains more than one PP headed by \textit{de}, as shown in (4a,b) from Milner (\citeyear{Milner1978,Milner1982}; quoted in \citealt{Sag1994}). Although the relative element \textit{dont} is generally exempt from the subject condition mentioned above,\footnote{See Footnote 1.} hence the grammaticality of (4a), the example in (4b) is ungrammatical:

\ea%4
    \label{ex:mensch:4}
    \ea[]{
    \gll M.  X  [\textsubscript{PP} dont]\textsubscript{} [\textsubscript{DP}    la  maison  [\textsubscript{PP}  de  {Le Corbusier}] \sout{[\textsubscript{PP} dont]}]  n’  est  guère  confortable.\\
    ...
    \ex[*]{
    \gll Le Corbusier [\textsubscript{PP} dont] [\textsubscript{DP}  la  maison \sout{[\textsubscript{PP}  dont]} [\textsubscript{PP}  de  M.  X]] n’  est  guère  confortable.\\
...
\ea%5
    ...
    \glt ‘Of which book do you know the end \_\_ ?’ (cf. \citealt{Sportiche1981}: 224)
    \ex  
    \gll \relax [\textsubscript{PP}  De quel linguiste]   avez-vous  rencontré [\textsubscript{DP}  les parents \sout{[\textsubscript{PP}  de quel  linguiste]}] ?\\
...
\ea%6
    \label{ex:mensch:6}
    \ea
    \gll la  maison  [\textsubscript{PP} dont / de laquelle]  vous avez  vu  [\textsubscript{DP}  une      photo \sout{[\textsubscript{PP} dont / de laquelle]}]\\
    ...
    \ex  
    \gll un   linguiste [\textsubscript{PP}  de qui / dont]    vous  avez  rencontré [\textsubscript{DP}  les parents \sout{[\textsubscript{PP} de qui / dont]}]\\
...
\ea%7
    \label{ex:mensch:7}
    \ea[*]{
    \gll \relax [\textsubscript{PP}  De  quel  pays]    avez-vous  rencontré [\textsubscript{DP} les arrivants  \sout{[\textsubscript{PP} de quel  pays]}] ?\\
        {} from  which  country  have-you  met {}    the arrivals {} from     which  country\\
    \glt ‘From which country have you met the arrivals \_\_’?  }
    \ex[*]{
    \gll Cette  prison, [\textsubscript{PP}  de laquelle] [\textsubscript{DP}  le transfert \sout{[\textsubscript{PP} de  laquelle]}  de      l’  accusé    au   tribunal]]]…\\
...
\ea%9
    \label{ex:mensch:9}
    \ea[*]{
    \gll La  jeune  femme   [\textsubscript{PP} dont] [\textsubscript{DP}  le  portrait [\textsubscript{PP} de Corot] \sout{[\textsubscript{PP} dont]}]   se  trouve  à  la  Fondation Barnes …\\
    ...
    \ex[]{
    \gll Corot [\textsubscript{PP} dont]  [\textsubscript{DP}  le portrait  \sout{[\textsubscript{PP} dont]} [\textsubscript{PP}  de cette  jeune        femme]]  se  trouve  à  la  Fondation Barnes …  \\
...
\ea%12
    \label{ex:mensch:12}
    \ea
    \gll La symphonie  [\textsubscript{PP} dont]  j’  aime  [\textsubscript{DP} l’  interprétation      [\textsubscript{PP} de Karajan]  \sout{[\textsubscript{PP} dont]}] ...\\
    ...
    \ex  
    \gll L’  histoire [\textsubscript{PP}  dont]     je   n’  ai  jamais  pu  avoir [\textsubscript{DP} la    version   [\textsubscript{PP}  de Marie] \sout{[\textsubscript{PP}  dont]}] ...\\
    ...
    \ex  
    \gll Les  événements [\textsubscript{PP}  dont]  j’ ai  apprécié [\textsubscript{DP} le  {compte rendu}     [\textsubscript{PP}  du Monde] \sout{[\textsubscript{PP}  dont]}]...\\
...
Spanish (\citealt{Gutiérrez-Bravo2001}: 111)\label{ex:mensch:17}\\
    \ea[]{
    \gll \relax [\textsubscript{PP} De  quién]  perdiste  [\textsubscript{DP}  la  traducción [\textsubscript{PP}  de  La    Odisea] \sout{[\textsubscript{PP}  de  quién]}] ?\\
     {}    of  whom  lost.\textsc{2sg} {}   the  translation {}    of  the        Odyssey {}  of  whom\\
    \glt ‘Of whom did you lose the translation of The Odyssey \_\_?’}
    \ex[*]{
    \gll \relax [ De qué] perdiste [\textsubscript{DP}  la  traducción [\textsubscript{PP}  de Juan] \sout{[\textsubscript{PP}  de qué]}]?\\
...
\ea%23
    \label{ex:mensch:23}
    \ea
    \gll \relax [\textsubscript{PP}  De qui]  as-tu  vu [\textsubscript{DP} \sout{[\textsubscript{PP}  de qui]} [\textsubscript{DP}   la  photo \sout{[\textsubscript{PP} de qui]}] ?\\
       {}  of whom  have-you  seen  {} {}  of whom {}  the  photo   {}     of whom\\
    \glt ‘Of whom have you seen the photo \_\_ ?’
    \ex  
    \gll le  prof [\textsubscript{PP}  dont]    j’  ai  vu [\textsubscript{DP} \sout{[\textsubscript{PP}  dont]} [\textsubscript{DP}  la   photo  \sout{[\textsubscript{PP} dont]}] \\
         the  professor {}  of.which  I  have  seen {} {}   of.which {}  the      photo  {}  of.which\\
    \glt ‘the professor of whom I have seen the photo \_\_’
    \ex  
    \gll (C’est) [\textsubscript{PP} DE JEAN]   (que)  j’ ai  vu   [\textsubscript{DP} \sout{[\textsubscript{PP} de Jean]}  la  photo \sout{[\textsubscript{PP} de Jean]}].\footnotemark\\
...
However, the provisional formalization in (22) clearly overgenerates: a D head with this feature composition could also attract the adjunct \textit{de qui} in the upper PP in (21) so as to yield the ungrammatical (24a) vs. the grammatical (24b). In fact, according to the descriptive generalization in (15), adjuncts can never be attracted, witness the PP indicating \textsc{source} in (24c) and an adjunct with another preposition as in (24d):

\ea%24
    \label{ex:mensch:24}
    \ea[*]{
    \gll \relax[\textsubscript{PP}  De  qui]  as-tu  vu  [\textsubscript{DP} \sout{[\textsubscript{PP}  de qui]} [\textsubscript{DP}   la  maison \sout{[\textsubscript{PP} de  qui]} [\textsubscript{PP}  de M. X]]] ?\\
      {}     of  whom  have-you  seen {} {}  of whom {}  the  house {}   of  whom {}   of Mr. X\\
    \glt     ‘Of whom have you seen the house \_\_ of Mr. X?’  }
    \ex[]{
    \gll \relax[\textsubscript{PP}  De  qui]  as-tu  vu [\textsubscript{DP} \sout{[\textsubscript{PP}  de qui]} [\textsubscript{DP}  la   maison     [\textsubscript{PP}  de Le Corbusier] \sout{[\textsubscript{PP}  de  qui]}]]?\\
        {} of  whom  have-you  seen  {} {}  of whom {} the  house  {}      of Le C. {}     of  whom\\
    \glt ‘Of whom have you seen the house of Le Corbusier \_\_?’ }
    \ex[*]{
    \gll \relax[\textsubscript{PP}  D’  où]  aimes-tu [\textsubscript{DP} \sout{[\textsubscript{PP}  d’  où]} [\textsubscript{DP} les  bananes \sout{[\textsubscript{PP}   d’   où]}]] ?\\
         {} from  where  like-you  {} {}  from  where {}  the  bananas {} from where\\
    \glt ‘From where do you like the bananas \_\_?’ }
    \ex[*]{
    \gll \relax[\textsubscript{PP}  Sur  qui]  as-tu  lu [\textsubscript{DP} \sout{[\textsubscript{PP}  sur  qui]} [\textsubscript{DP} un  livre \sout{[\textsubscript{PP} sur  qui]}]] ?\\
...
My research on extraction from DPs in French is funded by the Deutsche Forschungsgemeinschaft (DFG) in a project with Stefan Müller (Humboldt-Universität zu Berlin) and was also part of my activities within the Courant Research Centre “Text Structures” at the University of Göttingen.
% 
% \section{ References}
% 
