...
\todo{Following references not in bibliography: Yang, Borer, Terzi, Uchibori, Martins}
...
As the subject/object distinction has been lost for third person full pronouns in Brazilian Portuguese, the distinction between forward and backward object control can only be directly observed for the first person. (cf. \citealt{Farrell1995}; \citealt{Boeckx2004,Boeckx2006}):
...
\todo{Torrego not in bibliography}
\ea%8
    \label{ex:moreno:8}
    \ea  Romanian\\
...
First, unlike in the case of the ECM/raising construction, the passivization of the complement of \textit{fazer} and \textit{mandar} does affect the interpretation of the entire construction.\footnote{According to Jairo Nunes (p.c.), the alleged difference in meaning between (i) and (ii) seems to be no different from what we find in their subjunctive counterparts: 
...
\todo[inline]{Should the example mentioned above be (14)?}
\ea \gll  O   arquiteto mandou   ter    uma janela    em cada quarto.\\
the architect  ordered   have a      window in  each  room\\
\z The example Ferreira gives in (i) is similar to examples with the homophonous causative verb \textit{trimite} or ‘made’ in Romanian, which is syntactically distinct from the object control \textit{trimite/face:}
...
\todo[inline]{Huber not in bibliography}
\ea%18
...
\todo[inline]{Manzini et al. not in bibliography}
...
\todo[inline]{Which Hornstein reference? Also, Martins not in bib.}
...
\todo[inline]{Pires 2010 not in bibliography}
\ea%21
...
\todo[inline]{Which Hornstein? Martins not in bib}
\ea%25
...
\z
In the presence of the preposition that assigns inherent case to the inflected infinitive, the structural case of matrix verbs must be obligatorily realized by an accusative object realized in the matrix clause. Therefore, standard object control verbs that subcategorize prepositions allow only forward control patterns. They correspond to Prepositional Constructions (PC) in \citegen{Larson1981} terms: \textit{I gave a book to Mary.}
\todo[inline]{Which Larson?}
...
\todo[inline]{Alexiadou 2001 not in bib}
\begin{enumerate}
